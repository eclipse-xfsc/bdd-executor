...
spec:
  ...
  containers:
    - name: python
      ...
      # see `make docker-build-python-with-podman`  how to build it, TODO: build and push with GithubCI
    ...
    stages {
        ...
        stage("Test") {
            parallel {
                ...
                stage("Run bdd test on minimal-project") {
                    stages {
                        stage("download-selenium") {
                            steps {
                                ...
                                sh """#!/bin/bash
                                set -x -eu -o pipefail
                                cd "./implementations/minimal-project"
                                
                                export ARG_BDD_JUNIT="--junit --junit-directory='../../.tmp/behave/'"
                                make download-selenium
                                """
                            }
                        }

                        stage("docker-run-spring-boot-hello-world") {
                            steps {
                                // language=sh
                                sh """#!/bin/bash
                                set -x -eu -o pipefail
                                ...
                                sleep 60 # give time to setup
                                """
                            }
                        }

                        // use a jenkins image with java
                        stage("Temporary-install java") {
                            steps {
                                // language=sh
                                sh """#!/bin/bash
